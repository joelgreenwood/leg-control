...
int hipPot_pin = 20;
 ...
 //Pressor Sensors
int pressureSensorPin_1 = 28;
int pressureSensorPin_2 = 27;
int pressureSensorPin_3 = 26;
int pressureSensorPin_4 = A12;
...
for (int i = 0; i < 4; i++) {
 ...
 /*
 Serial.print("xyz joystick & Pots");
 Serial.print("\t");
*/
...
Serial.println(">>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<");
for (int i = 0; i < 4; i ++) {
  Serial.print(analogRead(pressure_pins[i]));
  Serial.print("\t");
 }
 Serial.println();
 Serial.println(">>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<");
...
//sensorPWMpressure[i + 11] = high_pwm_values[i];
//sensorPWMpressure[i + 12] = low_pwm_values[i];
