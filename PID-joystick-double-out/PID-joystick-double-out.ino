...
#define max_sensor_value 3600 // This is the maximum read of the string pot - in this case the photo resistor.  I will use the same numbers for forward and reverse to make it easier to convert to the string pot later.  
#define min_sensor_value 200 // This is the minimum read of the string pot - in this case the photo resistor. 
...
void setup()
{
  ...
  Input = map(lightLevel_forward, min_sensor_value, max_sensor_value, 0, ADC_num_of_bytes);
  lightLevel_reverse = adc->analogRead(photores_reverse_pin);
  Input = map(lightLevel_reverse, min_sensor_value, max_sensor_value, 0, ADC_num_of_bytes);
...
void loop()
{
  ...
  Setpoint = map(adc->analogRead(joystick_pin), 0, bits, 0, bits);


  lightLevel_forward = adc->analogRead(photores_pin);
  //Serial.print("LIGHTLEVEL - raw: ");
  //Serial.println(lightLevel_forward);
  Input = map(lightLevel_forward, min_sensor_value, max_sensor_value, 0, bits);
  myPID.Compute();
