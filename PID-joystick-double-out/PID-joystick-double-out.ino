...
#define min_sensor_value_forward 2585 // This is the minimum read of the string pot - in this case the photo resistor. 
...
void loop()
{
  ...
  if (Setpoint >= 0) {
    lightLevel_forward = adc->analogRead(photores_forward_pin);
    Input = map(lightLevel_forward, min_sensor_value_forward, max_sensor_value_forward, 0, ADC_num_of_bytes);
  }
  else {
    lightLevel_reverse = adc->analogRead(photores_reverse_pin);
    Input = map(lightLevel_reverse, min_sensor_value_reverse, max_sensor_value_reverse, 0, -ADC_num_of_bytes);
  }
  ...
  if(now - lastMessage > serialPing) {
        //Serial.print("raw forward sensor: ");
        //Serial.print((adc->analogRead(photores_forward_pin)));
        Serial.print("Setpoint: ");
        Serial.print(Setpoint);
        Serial.print(" Input: ");
        Serial.print(Input);
